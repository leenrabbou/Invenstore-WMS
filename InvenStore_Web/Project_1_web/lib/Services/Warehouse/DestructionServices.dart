...
import 'package:buymore/Models/Warehouse/Destruction%20Models/AnDestructionResponseModel.dart';
import 'package:buymore/Models/Warehouse/Destruction%20Models/DestructionCausesModel.dart';
import 'package:buymore/Models/Warehouse/Destruction%20Models/DestructionCausesResponseModel.dart';
import 'package:buymore/Models/Warehouse/Destruction%20Models/DestructionResponseModel.dart';
...
class DestructionServices {
  ...
  Future<DestructionResponseModel> showAllDestructionsService(
    ...
    String url;
    search == null ? url = 'destruction' : url = 'destruction?search=$search';
    final http.Response res = await http.get(
  ...
  Future<dynamic> addDestructionService(
      int id, int quantity, int causeId, Locale locale) async {
    var response = await http
        .post(Uri.parse('$basicurl/stored-products/$id/destruction'), headers: {
      'Authorization': 'Bearer $token',
      'locale': locale.languageCode
    }, body: {
      'quantity': quantity.toString(),
      'cause_id': causeId.toString(),
    });
