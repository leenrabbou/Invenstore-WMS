...
import 'package:buymore/Models/Warehouse/Shipment%20Models/AnShipmentResponseModel.dart';
import 'package:buymore/Models/Warehouse/Shipment%20Models/ShipmentResponseModel.dart';
...
class ShipmentServices {
  Future<ShipmentResponseModel> showAllShipmentsService(
      int page, String? search, Locale locale) async {
    String url;
    search == null
        ? url = 'shipments/?page=$page'
        : url = 'shipments/?page=$page&search=$search';
  ...
  Future<dynamic> addShipmentService(String driverName, String cost,
      int shippingCompanyId, int orderId, Locale locale) async {
    var response = await http.post(Uri.parse('$basicurl/shipments'), headers: {
      'Authorization': 'Bearer $token',
      'locale': locale.languageCode
    }, body: {
